...
def get_sales_invoice_items(result):
	...
	for d in result:
		...
		if d.account == ("'Opening'" or 'Opening'):
			d.voucher_no = 'Opening'
		if d.get('voucher_no') and d.get('against'):
			d.voucher_no = '{0}'.format(d.get('voucher_no'))
		...
		if d.get('voucher_type') in ['Sales Invoice', 'Purchase Invoice'] and d.get('voucher_no'):
			...
			voucher_items = frappe.db.sql(
				"""SELECT commercial_invoice_no, po_no, dc_no,net_total FROM `tab%s` """
				% (table_name), as_dict=1)
			if voucher_items:
				row = frappe._dict({
					'commercial_invoice_no':  f"{voucher_items[0].commercial_invoice_no}",
					'po_no':  f"{voucher_items[0].po_no}",
					'dc_no':  f"{voucher_items[0].dc_no}",
					'net_total':  f"{voucher_items[0].net_total}",
				})
